...
#thin output matrix for plotting.
out <- out[seq(1,nrow(out),by = 20),]
...
plot(out[,1]/out[,5], cex = 0.1, col='blue' , ylim = c(0,55))
par(new=T)
plot(out[,2]/out[,6], cex = 0.1, col='green', ylim = c(0,55))
par(new=T)
plot(out[,3]/out[,7], cex = 0.1, col='red'  , ylim = c(0,55))
#print C pools and their sums
results <- c(M,C,B,N2,N1,N3,N4,CUE_emergent,NUE_emergent)
results
