...
# Plant nitrogen uptake and leaching of inorganic N happen after microbial mineralization/immobilization.
# microbes aim to have the C:N ratio of uptake match their theshold element ratio (TER, biomass C:N / CUE)
...
for(i in 1:t){
  ...
  #save outputs
  out[i,] <- c(B,C,M,R,N1,N2,N3,N4,eco.C,eco.N)
}
...
par(new=T)
plot(out[,9]/out[,10], cex = 0.1, col='purple'  , ylim = c(0,40))
#ecosystem C and N balance
plot(out[, 9],ylab='ecosystem C')
plot(out[,10],ylab='ecosystem N')
...
model<- function(t,y,pars){
  with(as.list(c(y,pars)),{
    ...
    dCdt <- I + (DEATH.C - SORPTION.B.C) + DESORPTION.C - DECOMP.C - SORPTION.P.C
    dMdt <- SORPTION.C - DESORPTION.C
    #dRdt <- (1-CUE)*DECOMP.C + overflow.R
